...
// ─── Shared hero content — used by both desktop (parallax) and mobile (static) ───
function HeroContent({ prefersReduced }: { prefersReduced: boolean }) {
  // On mobile or when user prefers reduced motion, skip blur filter animation
  const h1Initial = prefersReduced
    ? { opacity: 0, y: 16 }
    : { opacity: 0, y: 32, filter: "blur(12px)" };
  const h1Animate = prefersReduced
    ? { opacity: 1, y: 0 }
    ...
    <div className="flex flex-col items-center text-center max-w-[860px] mx-auto">
      ...
      <motion.div
        ...
        {trustBadges.map(({ icon: Icon, value, label, color, dimColor, borderColor }) => (
          <div key={label}
            className="card flex flex-col items-center py-3.5 sm:py-4 px-3 gap-1 cursor-default hover:-translate-y-1 transition-transform duration-200"
            style={{ borderColor }}>
...
export default function Hero() {
  ...
  // Parallax — desktop only; mobile skips useScroll entirely
  ...
  return (
    <section ref={ref} id="hero" className="relative min-h-[100svh] lg:min-h-[90svh] flex flex-col justify-center overflow-hidden pt-[100px] sm:pt-[90px] pb-[32px] sm:pb-[48px]">
      ...
      {/* Static backgrounds — rendered for everyone */}
      <div className="absolute inset-0 z-0"
        style={{ background: "radial-gradient(ellipse 80% 55% at 50% -5%, rgba(124,111,255,0.1), transparent 60%)" }} />

      {/* Parallax bg — desktop (sm+) only */}
      <motion.div style={{ y: bgY }} className="absolute inset-0 z-0 pointer-events-none hidden sm:block">
      ...
      {/* Static bg — mobile */}
      <div className="absolute inset-0 z-0 pointer-events-none sm:hidden">
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 500px 400px at -5% 10%, rgba(124,111,255,0.12), transparent 65%)" }} />
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 400px 350px at 105% 90%, rgba(34,211,238,0.06), transparent 65%)" }} />
      </div>

      {/* Aurora — desktop only (animated, GPU-heavy) */}
      <div className="absolute left-1/2 top-[38%] -translate-x-1/2 -translate-y-1/2 w-[900px] h-[450px] pointer-events-none z-[1] hidden sm:block">
        <motion.div className="absolute inset-0 rounded-full"
          style={{ background: "radial-gradient(ellipse at center, rgba(124,111,255,0.25), transparent 65%)", filter: "blur(100px)" }}
          animate={prefersReduced ? {} : { opacity: [0.35, 0.7, 0.35], scale: [0.92, 1.06, 0.92] }}
          transition={{ duration: 9, repeat: Infinity, ease: "easeInOut" }} />
      </div>
      {/* Static glow — mobile (no animation = no jank) */}
      <div className="absolute left-1/2 top-[38%] -translate-x-1/2 -translate-y-1/2 w-[340px] h-[280px] pointer-events-none z-[1] sm:hidden rounded-full"
        style={{ background: "radial-gradient(ellipse at center, rgba(124,111,255,0.18), transparent 65%)", filter: "blur(60px)" }} />
      ...
      {/* Desktop: text inside parallax motion.div */}
      <motion.div style={{ y: txtY }} className="container relative z-10 hidden sm:block">
        <HeroContent prefersReduced={!!prefersReduced} />
      </motion.div>

      {/* Mobile: text as plain div — no useScroll, no motion value subscription, no jank */}
      <div className="container relative z-10 sm:hidden">
        <HeroContent prefersReduced={true} />
      </div>
